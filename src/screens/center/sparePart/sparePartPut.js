...
export default SparePartPut = ({ route }) => {
  ...
  const [maintenanceServiceName, setMaintenanceServiceName] =
    useState(sparePartsItemName);
  ...
  const handleSignup = async () => {
    try {
      ...
      const response = await axios.put(
        `http://autocare.runasp.net/api/SparePartItem/Update?id=${sparePartsItemId}`,
        {
      ...
      if (response.status === 200) {
        setLoad(false);
        alert("sửa phụ tùng thành công!");
        navigation.navigate("PRODUCT");
      } else {
        alert("sửa phụ tùng không thành công. Vui lòng thử lại.");
      }
    } catch (error) {
      ...
      <View style={styles.inputContainer}>
        ...
        {load ? (
          ...
          <Pressable style={styles.submitButton} onPress={handleSignup}>
            <Text style={styles.submitButtonText}>Sửa phụ tùng</Text>
          </Pressable>
        )}
      </View>
    </View>
