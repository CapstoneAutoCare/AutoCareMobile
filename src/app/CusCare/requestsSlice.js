...
export const fetchRequests = createAsyncThunk('requests/fetchRequests', async (centreId) => {
  ...
  const response = await Axios.get(
    `http://solv2.runasp.net/api/Bookings/GetListByCenterId?id=${centreId}`,{
      headers: {
...
const fetchClientName = async (clientId) => {
  const response = await Axios.get(`http://solv2.runasp.net/api/Clients/GetById?id=${clientId}`);
  return `${response.data.firstName} ${response.data.lastName}`;
};

const fetchVehicleNumber = async (vehicleId) => {
  const response = await Axios.get(`http://solv2.runasp.net/api/Vehicles/GetById?id=${vehicleId}`);
  return response.data.licensePlate;
};

const fetchMaintenanceCenterName = async (maintenanceCenterId) => {
  const response = await Axios.get(`http://solv2.runasp.net/api/MaintenanceCenters/GetById?id=${maintenanceCenterId}`);
